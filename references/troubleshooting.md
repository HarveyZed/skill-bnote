...
**`validation.json` 怎么读**：结果在 `result`（`pass`/`fail`）、条目分三个数组——`errors`（拦住覆盖成品、进派修队列）、`warnings`（只提醒、不拦流程）、`review`（审阅发现，带 `severity`）。每条带 `level`（`error` / `warning` / `review`，与所在数组一致）、`owner`（谁来修）、`chapter`/`file`、`message`、`fix_hint`。**判断流程是否被拦住只看数组与 `result`**，不要只看 `level`。
...
| 校验报告里"有若干张幻灯片没有被正文引用"看着像错误 | 它本来就是**警告**（`result` 仍为 `pass`、`errors` 为空）。0.9.1 起 `level` 字段统一为 `error`/`warning`；**更早版本生成的 `validation.json` 里这条会被误标成 `error`**，重跑一次 `check`/`merge` 就会刷新成正确值 |
...
| 置顶评论长期取不到，换了视频也一样 | 先排除本地原因（网络、超时、`meta.top_comment` 是否被关掉）。若都正常，多半是**上游接口变化或风控加强**——评论接口不是公开接口，平台改版不通知，这类失效**不是本地能修的**，只能等适配；在此期间取数照常，只是少了这段背景。别把它当成流程故障去反复重跑 |
